...
subtitle: 'Labs created by Adam Garber & Dina Arch, taken from Nylund et al. (2022)'
...
step1_fit <- mplusModeler(step1,
                            dataout=here("cov_lta", "t1.dat"),
                            modelout=here("cov_lta","one_T1.inp") ,
                            check=TRUE, run = TRUE, hashfilename = FALSE)
```
...
step1_fit <- mplusModeler(step1,
                            dataout=here("cov_lta", "t2.dat"),
                            modelout=here("cov_lta","one_T2.inp") ,
                            check=TRUE, run = TRUE, hashfilename = FALSE)
```
...
output_T1 <- readModels(here("cov_lta","one_T1.out"))
...
output_T2 <- readModels(here("cov_lta","one_T2.out"))
...
step3_fit <- mplusModeler(step3,
               dataout=here("cov_lta", "three.dat"), 
               modelout=here("cov_lta", "three.inp"), 
               check=TRUE, run = TRUE, hashfilename = FALSE)
```
...
lta_model <- readModels(here("cov_lta", "three.out"))
...
cov <- as.data.frame(lta_model[["parameters"]][["unstandardized"]]) %>%
  filter(param %in% c("SCI_ISSU", "SCI_IRT7", "FEMALE")) %>% 
  mutate(param = case_when(
            param == "SCI_ISSU" ~ "Interest in Science Issues",
            param == "SCI_IRT7" ~ "Science IRT Score",
            param == "FEMALE" ~ "Gender"),
    se = paste0("(", format(round(se,2), nsmall =2), ")"),
    paramHeader = case_when(
            paramHeader == "C1#1.ON" ~ "Pro-Science",
            paramHeader == "C1#2.ON" ~ "Amb. / Elev. Utility",
            paramHeader == "C1#3.ON" ~ "Amb. / Min. Utility")
         ) %>% 
  unite(estimate, est, se, sep = " ") %>% 
  select(paramHeader:pval, -est_se) %>% 
...
cov_m1 <- cov %>% 
  gt(groupname_col = "param", rowname_col = "paramHeader") %>%
  tab_header(
    title = "Relations Between the Covariates and Latent Class  (T1)") %>%
  tab_footnote(
    footnote = md(
